// Cubism.js customized
// 
// MOD20140214KM - remove any %p formatting styles to create 24hour time ruler
...
cubism_contextPrototype.graphite = function(host) {
  ...
  source.metric = function(expression) {
	...
	url = host + "/render?format=raw"
          ...
          + "&until=" + cubism_graphiteFormatDate(stop - 1000)

	console.log("GRAPHITE URL :"+url)
	...
	console.log("graphite returned data: "+text)
...
result = text
          .substring(i + 1)                                                                                                                                                                   .split(",")                                                                                                                                                                         .slice(1) 

          console.log("graphite parse result "+result)
          // the first value is always None?  
          return result.map(function(d) { return +d; });
...
var cubism_axisFormatSeconds = d3.time.format("%H:%M:%S"), //MOD20140214KM
    cubism_axisFormatMinutes = d3.time.format("%H:%M"), //MOD20040214KM
