...
function merge_ranges(a,b) 	{ return [Math.min(a[0],b[0]), Math.max(a[1],b[1])] }

_new = [];_copy = scales.slice(0) /*copy*/; uniq_scales = scales

if (scales.length > 2) { 
	for (i = 0; i < scales.length - 1; i++ ) { 
		for (j = 1; j < scales.length; j++ ) { 
			if (i != j) { 
				a = scales[i]; b = scales[j]
				if (overlap(a,b)) { 
					c = merge_ranges(a,b)
					scales[i] = c
					scales[j] = c
					_copy[i]=null; _copy[j] = null; _new.push(c)
				}
			}	
		}
	}
	...
	_new2 = _.map(_new, function(d) { return ""+d[0]+"|"+d[1]})
	_cpy2 = _.map(_copy, function(d) { return ""+d[0]+"|"+d[1]})
	
	uniq_scales = _.map(_.unique(_.compact(_new2.concat(_cpy2))), function(d) { return d.split("|") })
}

// Make d3 scales for each scale array
d3_scale = []; $.each(scales, function(i,d) { d3_scale.push(d3.scale.linear().domain([d[0], d[1]]).nice()) })
uniq_d3_scale = []; $.each(uniq_scales, function(i,d) { uniq_d3_scale.push(d3.scale.linear().domain([d[0], d[1]]).nice()) })


// Push scales and their metrics into a rickshaw-eatable array
series = []
for (n = 0; n < data.length; n++) {
        series.push( { color: colours[n], 
                        data: data[n].data,
			name: data[n].name,
			scale: d3_scale[n],
        })
...
function generate_legend(date, v) { 
	legend.innerHTML = "<table><tr><td colspan=3>"
		+ date
		+ "</td></tr>" 
		+ $.map(v, function(d) { 
...
pad = []
for (i = 0; i < uniq_scales.length; i++) { pad.push(1) }
